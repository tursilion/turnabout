...
Tursi has made an addition that a .lowertext section is assembled for
the lower 8k block, and will be appended to the file with its own
EA5 header that the EAsplit tool can deal with. The lowertext section
does not have any provision for initialized data (and probably should
not have any, I'm not sure if that won't overflow the upper bank
initialized data.)
*/
...
*/
   ...
   if (lower_text_section == NULL) {
       ea5_header[0] = 0;                                // Last in file chain (0000=yes, ffff=no) 
   } else {
       ea5_header[0] = 0xffff;
   }
   ea5_header[1] = convert2(image_size);             // Image + header size
   ea5_header[2] = convert2(text_section->sh_addr);  // Load address
   fwrite(ea5_header, 6, 1, output);
   printf("Writing ea5 with size 0x%04X load 0x%04X\n", image_size+6, text_section->sh_addr);
   ...
   // now handle .lowertext
   if (lower_text_section != NULL) {
       // Calculate image size (.text + .data)
       image_size = lower_text_section->sh_size;

       // Write EA5 header
       //
       // The EA5 loader assumes the entry point is the load address
       ea5_header[0] = 0;                                // Last in file chain (yes)
       ea5_header[1] = convert2(image_size);             // Image + header size
       ea5_header[2] = convert2(lower_text_section->sh_addr);  // Load address
       fwrite(ea5_header, 6, 1, output);
       printf("Appending ea5 with size 0x%04X load 0x%04X\n", image_size+6, lower_text_section->sh_addr);

       // Copy all .lower_text section data
       fseek(input, lower_text_section->sh_offset, SEEK_SET);
       size = lower_text_section->sh_size;
